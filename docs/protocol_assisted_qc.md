...
5. 无需填写识别依据。点击“预览同类影响”，核对人数与人工冲突，再“确认识别并应用同类”。
   ...
   候选）”，然后使用“预览同类影响”。这个入口同样允许人工排除预览失败的模板，只作用于
   明确设为移出的模板，不会顺带排除其他未勾选序列。手动待定仍受保护；若同一模板既被
   勾选待定又被设为移出，须先解决这个冲突。预览列出实际排除名称，确认发布后才生效。
...
同一患者的有效 T1 候选仅由 SAG、TRA 两类组成且两类同时存在时，默认优先 TRA。
只有一幅 TRA 才自动完成 T1 识别；多幅 TRA（含同名重扫）、其他未解决的 T1 协议或
方向词本身冲突时继续人工比较。不会跨患者借用同组其他人的 TRA。

名称匹配不要求 `t1` 与 `sag`/`tra` 相邻，兼容大小写、空格、下划线、连字符和编号后缀，
例如 `eT1W-3D-SAG320-extra`、`T1__SE__TRA320`。明确的 `PosDisp` 导出后缀不参与
方向词匹配，例如 `T1-se-sag-320--PosDisp-t2-tse-tra-384` 仍按名称识别为 SAG。
`contrast` 中的 `tra` 不视为方向词；不同字段不拼接关键词。

SAG 仍属于 T1，不删除、不标为质量不通过，也不自动写入人工排除规则。
候选总数保留原值，唯一首选计入 `automatic_unique`；选择依据为 `t1_tra_over_sag`，
同组展示的默认协议优先级为 TRA 0、SAG 100。人工规则、人工影像决定、已选最终候选、
目标待定/读取失败和主动重审优先于这条默认规则。整轮不可读的技术跳过另见下文。

更新后运行原 `catalog`，将默认版本升级到 `sequence-defaults-4`，自动备份旧识别状态，
