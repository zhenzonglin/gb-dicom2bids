...
“本轮待确认的序列”中的待定框只由审核者手动勾选或取消。预览失败、切换展示窗、
刷新或重试均不会替您新增勾选，也不会锁定该框。以前已经保存的待定选择仍会恢复；
旧记录没有区分当时的勾选来源，因此升级不擅自删除，可以手动调整后预览并发布。
预览失败仍单独提示，但不会阻拦人工序列排除。手动勾选不能确定的项，再点击“本轮序列
均不是目标模态”并核对、发布影响：未勾选项全部纳入同组模板排除，包括预览失败项。
仅取消勾选不会自动排除，仍需预览并发布；这不是质量通过或质量不通过，也不删除文件。
...
新增模板会使原先自动跳过的患者重新待识别。手动勾选“待定”的项不参与排除；同模板有
任何手动待定项时，本轮暂不发布该模板。待定可随时取消后重新预览、发布。新发布的整轮
排除使用 `identity_only` 依据，不要求影像预览成功；失败日志保留，重启后不会仅因该错误
把已明确排除的同组模板重新放回队列。质量检查和 BIDS 归档仍保留原有技术校验。
旧规则不追溯放宽；需要采用新行为时，由审核者在网页重新预览并确认发布。规则及其历史
记录保存依据版本、预览失败项和可取得的源文件时间/大小，用于审计和发布前变更检查。
