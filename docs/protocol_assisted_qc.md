...
不同协议或同模板重复扫描均计为多候选，不自动择一。人工按组确认协议与优先级后，
真实重扫仍保留多个影像，留到质量阶段比较。
...
确认 T1 不完成 FLAIR，确认序列不等于图像质量通过。同模板重扫仍留待质量阶段逐幅比较。
...
唯一 T1 的计数也包含 T1-FLAIR；普通 T1 与 T1-FLAIR 并存不能自动选一幅。
**识别完成不是质量通过**，质量模型和独立验证门槛、人工质量确认及单独 `--apply` 不变。

#### T1 / FLAIR 的 OAx、TRA 轴位默认优先级

`OAx`、`TRA` 表示名称中的轴位，`OSag`、`SAG` 表示矢状位。分别对同一患者的 T1、FLAIR
处理：有效目标候选仅由这两种方向组成且同时存在时，优先唯一一幅轴位影像，自动完成该
模态识别。只有一个明确候选时仍沿用唯一识别规则。多幅轴位（含同名重扫）、其他未解决
的目标协议或方向词本身冲突时继续比较，不跨患者借用同组其他人的轴位影像。
...
候选总数保留原值，唯一首选计入 `automatic_unique`；选择依据分别为 `t1_tra_over_sag`
和 `flair_tra_over_sag`，兼容旧字段名。同组默认协议优先级为轴位 0、矢状位 100。
人工规则、人工影像决定、已选最终候选、
目标待定/读取失败和主动重审优先于这条默认规则。整轮不可读的技术跳过另见下文。

同字段 ≥4 个的模态跳过规则仍先于方向优先级，不能借轴位首选绕过。
更新后运行原 `catalog`，将默认版本升级到 `sequence-defaults-5`，自动备份旧识别状态，
