...
async function save(next=false){if(identifying()){message('此阶段只保存序列规则，请点击“预览同类影响”后确认识别。',true);return;}if(!draft)return;try{draft.reviewer=$('#reviewer').value;for(const modality of ['t1','flair'])if(draft.groups[modality])draft.groups[modality].reason=$('#none-reason-'+modality).value;draft=await api('/api/save',{subject:current.subject,decision:draft});dirty=false;$('#revision').textContent=`记录版本 ${draft.revision}`;message('决定已保存。staging尚未改变；需在终端执行应用。');if(next){const index=listing.findIndex(s=>s.id===current.subject);if(index+1<listing.length)await openSubject(listing[index+1].id);else{offset+=100;current=null;await refreshList();}}}catch(error){message(error.message,true);}}
