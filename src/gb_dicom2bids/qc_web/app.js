  ...
  try{
    ...
    for(const subject of listing){
      ...
      if(subject.identification_group){
        button.append(element('small',`${subject.center} · ${subject.modality.toUpperCase()} · 同类 ${subject.count} 人`),element('small',`待识别 ${subject.pending_count} 人 · ${subject.reason==='missing'?'缺少目标候选，请从备选纠错':'仅识别序列，不判断质量'}`));
    ...
    status.textContent=data.total?`列表已加载 · ${data.total} ${data.unit==='protocol_groups'?'个序列组':'名患者'}`:identifying()?'本队列无待识别组；可查看全部协议组纠错，或进入质量检查。':'当前筛选无匹配患者，请清除筛选条件。';
...
function exclusionLabel(candidate){const modes=candidate.excluded_modalities||[];if(candidate.default_excluded)return '默认非目标 · '+(candidate.default_classification?.reason||'');return modes.length?'已排除于 '+modes.map(m=>m.toUpperCase()).join('/') : '';}
