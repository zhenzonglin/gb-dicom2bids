...
function comparisonCandidates(list){const chosen=[];const modality=c=>draft.candidates[c.id]?.modality||c.candidate_type;const group=current.identification_groups?.find(g=>g.id===activeIdentificationGroup);if(identifying()&&group){chosen.push(...list.filter(c=>modality(c)===group.modality).slice(0,2));}for(const target of ['t1','flair']){if(chosen.length===2)break;const preferred=draft.groups[target]?.choice||current.sequence_choices?.[target];const candidate=list.find(c=>c.id===preferred&&!chosen.includes(c))||list.find(c=>modality(c)===target&&!chosen.includes(c));if(candidate)chosen.push(candidate);}for(const candidate of list){if(chosen.length===2)break;if(!chosen.includes(candidate))chosen.push(candidate);}return chosen;}
