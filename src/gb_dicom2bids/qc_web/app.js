...
async function refreshList(preferredGroup=null){
  ...
  try{
    ...
    for(const subject of listing){
      ...
      if(subject.identification_group){
        button.append(element('small',`${subject.center} · ${subject.modality.toUpperCase()} · 同类 ${subject.count} 人`),element('small',subject.reason==='all_candidates_unreadable'?'全部待选不可读 · 技术跳过，可重试':`待识别 ${subject.pending_count} 人 · ${subject.reason==='missing'?'缺少目标候选，请从备选纠错':'仅识别序列，不判断质量'}`));
      }else button.append(element('small',`${subject.center} · T1 ${subject.t1} / FLAIR ${subject.flair} / 其他 ${subject.other}`),element('small',`已评 ${subject.reviewed} · 已定模态 ${subject.resolved}${subject.pilot?' · pilot':''}`));
...
async function buildViews(grid,tools,candidate,meta,epoch){
  ...
  slider.oninput=draw;
  for(const k of candidate.assist?.suspect_slices||[]){
    const jump=element('button',`可疑层 ${k+1}`);
    jump.onclick=()=>{slider.value=k;draw();};
    tools.append(jump);
  }
  frame.onwheel=event=>{
...
async function save(next=false){if(identifying()){message('此阶段只保存序列规则，请直接确认识别；保存结果见后台任务状态。',true);return;}if(!draft)return;try{draft.reviewer=$('#reviewer').value;for(const modality of ['t1','flair'])if(draft.groups[modality])draft.groups[modality].reason=$('#none-reason-'+modality).value;draft=await api('/api/save',{subject:current.subject,decision:draft});dirty=false;$('#revision').textContent=`记录版本 ${draft.revision}`;message('决定已保存。staging尚未改变；需在终端执行应用。');if(next){const index=listing.findIndex(s=>s.id===current.subject);if(index+1<listing.length)await openSubject(listing[index+1].id);else{offset+=100;current=null;await refreshList();}}}catch(error){message(error.message,true);}}
let timer;['search','center','protocol','reason-filter'].forEach(id=>$('#'+id).oninput=()=>{clearTimeout(timer);timer=setTimeout(()=>{offset=0;refreshList();},350);});['auto-status','pilot','pending'].forEach(id=>$('#'+id).onchange=()=>{offset=0;refreshList();});$('#others').onchange=()=>{if(identifying()){const all=$('#identify-show-all');if(all){all.checked=$('#others').checked;all.onchange();}return;}offset=0;refreshList();if(current)renderPanes();};$('#prev-page').onclick=()=>{offset=Math.max(0,offset-100);refreshList();};$('#next-page').onclick=()=>{offset+=100;refreshList();};document.querySelectorAll('[data-none]').forEach(button=>button.onclick=()=>{const m=button.dataset.none;draft.groups[m]={choice:null,none:true,reason:$('#none-reason-'+m).value};changed();});document.querySelectorAll('[data-clear]').forEach(button=>button.onclick=()=>{delete draft.groups[button.dataset.clear];changed();});['reviewer','none-reason-t1','none-reason-flair'].forEach(id=>$('#'+id).oninput=changed);$('#save').onclick=()=>save();$('#save-next').onclick=()=>save(true);window.onbeforeunload=event=>{if(dirty){event.preventDefault();event.returnValue='';}};document.addEventListener('keydown',event=>{if(event.ctrlKey&&event.key==='s'){event.preventDefault();save();}});refreshList();
$('#assist-queue').onchange=()=>{if(dirty&&!confirm('放弃尚未保存的修改并切换队列？'))return;dirty=false;current=null;activeIdentificationGroup=null;offset=0;refreshList();};
$('#retry-list').onclick=()=>refreshList();
$('#next-stage').onclick=transitionStage;
